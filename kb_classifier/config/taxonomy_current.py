...
def load_current_taxonomy(
    ...
    If ``version`` is given, that specific ``taxonomy_v<version>[_<count>].py``
    is loaded and pinned (raising if it does not exist). Stage B uses this to
    freeze a production taxonomy version so it does not drift as Stage A keeps
    generating new rounds.
    """
    paths = SETTINGS.paths
    config_dir = config_dir or paths.config_dir

    # 0. explicit pinned version (Stage B production freeze)
    if version is not None:
        path = _taxonomy_path_for_version(config_dir, version)
        if path is None:
            raise FileNotFoundError(
                f"no taxonomy file for pinned version v{version} in {config_dir}"
            )
        tax = _load_taxonomy_from_file(path)
        validate_taxonomy(tax)
        label = os.path.basename(path)
        log.info("[taxonomy] pinned taxonomy = %s", label)
        return tax, label
