...
Usage
-----
  python -m kb_classifier.run_bootstrap --limit 30000     # round on 30k
  python -m kb_classifier.run_bootstrap --limit 100000    # next round on 100k
  python -m kb_classifier.run_bootstrap --limit 100000 --rediscover-all
  python -m kb_classifier.run_bootstrap --dry-run         # no LLM calls
"""
...
from .common import anchors as anchors_mod
from .common import matching as matching_mod
...
from .common.corpus import load_manifest, iter_documents
from .common.embedder import BgeM3Embedder, existing_shard_starts, load_all_embeddings, plan_shards
...
def _ensure_embeddings_complete(cfg, need_docs: Optional[int] = None) -> int:
    ...
    if missing:
        log.error("[run] embedding cache is INCOMPLETE for a scope of %d docs "
                  ...
                  "`python -m kb_classifier.run_embed embed` first.",
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="python -m kb_classifier.run_bootstrap",
