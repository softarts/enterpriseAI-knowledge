...
-----------
  ...
  python -m kb_classifier.run_embed manifest

  # ... or a stratified sample
  python -m kb_classifier.run_embed manifest --max-docs 30000

  # 2. Measure throughput on this machine before committing to a full pass
  python -m kb_classifier.run_embed benchmark --shards 3

  # 3. Embed. Ctrl-C at any time; re-run the same command to continue.
  python -m kb_classifier.run_embed embed
  python -m kb_classifier.run_embed embed --time-budget-min 90

  # 4. Check where you are without loading the model
  python -m kb_classifier.run_embed status
"""
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="python -m kb_classifier.run_embed",
